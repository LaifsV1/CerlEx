%% Modified for CerlEx (2026): NIF stubs for member/2, reverse/2, keyfind/3,
%% keymember/3 and keysearch/3 replaced by pure Erlang, and map uses in seq/3
%% and uniq/1,2 removed.
%%
%% %CopyrightBegin%
%%
...
keymember(Key, N, [H|_]) when element(N, H) =:= Key -> true;
keymember(Key, N, [_|T]) -> keymember(Key, N, T);
keymember(_, _, []) -> false.
...
keysearch(Key, N, [H|_]) when element(N, H) =:= Key -> {value, H};
keysearch(Key, N, [_|T]) -> keysearch(Key, N, T);
keysearch(_, _, []) -> false.
...
uniq_1([X | Xs], Seen) ->
    case member(X, Seen) of
        true  -> uniq_1(Xs, Seen);
        false -> [X | uniq_1(Xs, [X | Seen])]
    end;
uniq_1([], _) ->
    [].
...
uniq_2([X | Xs], F, Seen) ->
    Key = F(X),
    case member(Key, Seen) of
        true  -> uniq_2(Xs, F, Seen);
        false -> [X | uniq_2(Xs, F, [Key | Seen])]
    end;
uniq_2([], _, _) ->
    [].
