...
let rec pp_literal ppf lit =
  ...
  | Lit_list lits -> fprintf ppf "[%a]"
    (pp_print_comma_list pp_literal) lits
  | Lit_map kvs ->
    let pp_kv ppf (k, v) = fprintf ppf "%a=>%a" pp_literal k pp_literal v in
    fprintf ppf "~{%a}~" (pp_print_comma_list pp_kv) kvs
...
and pp_pattern ppf p =
  ...
  | Pat_alias (v, p) -> fprintf ppf "%a = %a"
    pp_var v
    pp_pattern p
  | Pat_map ps ->  (* CerlEx: map pattern ~{k:=pat,...}~ *)
    let pp_entry ppf { mpa_key; mpa_pat } =
      fprintf ppf "%a:=%a" pp_expr mpa_key pp_pattern mpa_pat in
    fprintf ppf "~{%a}~" (pp_print_comma_list pp_entry) ps
...
and pp_expr ppf e =
  ...
  | Expr_catch e -> fprintf ppf "catch %a"
    pp_expr e
  | Expr_map { map_base; map_assocs } ->  (* CerlEx: map literal or update expression *)
    let pp_op ppf = function MapAssoc -> pp_print_string ppf "=>"
                           | MapExact -> pp_print_string ppf ":=" in
    let pp_assoc ppf { ma_key; ma_val; ma_op } =
      fprintf ppf "%a%a%a" pp_expr ma_key pp_op ma_op pp_expr ma_val in
    begin match map_base with
    | None   -> fprintf ppf "~{%a}~" (pp_print_comma_list pp_assoc) map_assocs
    | Some b -> fprintf ppf "~{%a|%a}~" (pp_print_comma_list pp_assoc) map_assocs pp_expr b
    end
