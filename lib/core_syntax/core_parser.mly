...
(* Map syntax: ~{...}~ with => (MapAssoc) and := (MapExact) *)
%token <Parse_info.t> TILDE MAP_ASSOC MAP_UPDATE
...
let constant :=
    ...
    | TILDE; LEFT_BRACE; RIGHT_BRACE; TILDE; { Lit_map [] } (* CerlEx: empty map constant ~{}~ *)
    | TILDE; LEFT_BRACE; kvs = separated_nonempty_list(COMMA, map_const_entry); RIGHT_BRACE; TILDE; { Lit_map kvs } (* CerlEx: map constant ~{k=>v,...}~ *)
...
let pattern :=
    ...
    | TILDE; LEFT_BRACE; RIGHT_BRACE; TILDE; { Pat_map [] } (* CerlEx: empty map pattern ~{}~ *)
    | TILDE; LEFT_BRACE; ps = separated_nonempty_list(COMMA, map_pat_entry); RIGHT_BRACE; TILDE; { Pat_map ps } (* CerlEx: map pattern ~{k:=pat,...}~ *)
...
let bitstring_pattern :=
    ...
    | LEFT_PARENS; seg = bitstring_pattern; DASH_PIPE; LEFT_BRACKET; _ = separated_list(COMMA, constant); RIGHT_BRACKET; RIGHT_PARENS; { seg } (* CerlEx: accept annotated bitstring pattern segment (#<V>(params) -| [...]); OTP emits {segment,N} on each segment *)
...
let single_expression :=
    ...
    | e = map_expr; { e } (* CerlEx: map literal/update ~{k=>v,...}~ or ~{k=>v,...|base}~ *)
...
let bitstring :=
    ...
    | LEFT_PARENS; seg = bitstring; DASH_PIPE; LEFT_BRACKET; _ = separated_list(COMMA, constant); RIGHT_BRACKET; RIGHT_PARENS; { seg } (* CerlEx: accept annotated bitstring expression segment (#<V>(params) -| [...]); OTP emits {segment,N} on each segment *)
...
let prim_op_call :=
    ...
    | PRIMOP; LEFT_PARENS; pop_name = atom; DASH_PIPE; LEFT_BRACKET; _ = separated_list(COMMA, constant); RIGHT_BRACKET; RIGHT_PARENS; LEFT_PARENS; pop_args = separated_list(COMMA, expression); RIGHT_PARENS; { (pop_name, pop_args) } (* accept annotated primop name: primop ('name' -| [...]) (args) *)
...
(* CerlEx: map expression — empty, creation, or update with optional base *)
let map_expr :=
    | TILDE; LEFT_BRACE; RIGHT_BRACE; TILDE;
        { Expr_map { map_base = None; map_assocs = [] } }
    | TILDE; LEFT_BRACE; assocs = separated_nonempty_list(COMMA, map_assoc_entry); RIGHT_BRACE; TILDE;
        { Expr_map { map_base = None; map_assocs = assocs } }
    | TILDE; LEFT_BRACE; assocs = separated_nonempty_list(COMMA, map_assoc_entry); PIPE; base = expression; RIGHT_BRACE; TILDE;
        { Expr_map { map_base = Some base; map_assocs = assocs } }

(* CerlEx: map association entry — k => v (upsert) or k := v (exact update, key must exist) *)
let map_assoc_entry :=
    | k = expression; MAP_ASSOC;  v = expression; { { ma_op = MapAssoc; ma_key = k; ma_val = v } }
    | k = expression; MAP_UPDATE; v = expression; { { ma_op = MapExact; ma_key = k; ma_val = v } }

(* CerlEx: map pattern entry — k := pat (key must be present; binds value to pat) *)
let map_pat_entry :=
    | k = expression; MAP_UPDATE; p = annotated_pattern; { { mpa_key = k; mpa_pat = p } }

(* CerlEx: map constant entry — k => v (literal keys and values only, no base) *)
let map_const_entry :=
    | k = constant; MAP_ASSOC; v = constant; { (k, v) }
