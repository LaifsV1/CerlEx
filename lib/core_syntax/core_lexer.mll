...
rule read = parse
    ...
    | "=>" { MAP_ASSOC  (tokinfo lexbuf) } (* CerlEx: map assoc operator *)
    | ":=" { MAP_UPDATE (tokinfo lexbuf) } (* CerlEx: map exact-update / pattern-bind operator *)
    | "~"  { TILDE      (tokinfo lexbuf) } (* CerlEx: map delimiter prefix/suffix in ~{...}~ *)
