...
%% +++++++++++++++++++++++ hash/1
%% A small, deterministic string hash.  Keeping it in Erlang gives CerlEx and
%% BEAM exactly the same key-space ordering without relying on VM-specific
%% implementations of erlang:phash/2.
hash(String) ->
	hash_chars(String, 5381).

hash_chars([], Hash) ->
	Hash;
hash_chars([Char|Rest], Hash) ->
	hash_chars(Rest, ((Hash * 33) + Char) rem 4294967296).
